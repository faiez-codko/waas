  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-white via-zinc-50 to-zinc-100 dark:from-black dark:via-zinc-900 dark:to-zinc-950">
      <div className="w-full max-w-md rounded-xl bg-white p-8 shadow-md dark:bg-zinc-900">
        <h2 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50">Create an account</h2>
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">Start using WaaS with your email.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <div>
            ...
            <input value={confirm} onChange={(e) => setConfirm(e.target.value)} type="password" className="mt-1 w-full rounded-md border border-zinc-200 px-3 py-2 bg-white text-sm" />
          </div>

          {error && <div className="text-sm text-red-600">{error}</div>}

          <button type="submit" className="w-full rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">Create account</button>
        </form>

        <div className="mt-4 text-sm text-zinc-600">Already have an account? <a href="/login" className="text-indigo-600">Sign in</a></div>
      </div>
    </div>
  );
}
